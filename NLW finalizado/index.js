...
const updateSubcribers = (userData) => 
{
...
const showInvate = (userData) =>
{
  app.innerHTML =` 
      ...
      <main>
        ...
        <section class="stats">
          <h4>
            ${updateSubcribers(userData)}
          </h4>
          <p>Inscricoes feitas</p>
        </section>`


  app.setAttribute('class', 'pagei-invite')
 updateImageLinks()
}

const abeUser = (userData) =>
{
  const newUser = {
    ...userData,
...
const formAction = () => {
  ...
  form.onsubmit = (event) =>
  {
    ...
    if(user)
    {
      showInvate(user)//find
    }
    else{
      const newUser = abeUser(userData)//notfind
      showInvate(newUser)
    }
